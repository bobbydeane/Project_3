...
row_count = len(word_meaning_examples.col_values(1))
# counts all rows with data entries in col1
row_ref_start = row_count + 1
# accounts for Sheets rows starting at 1
word = "word"
meaning = "meaning"
word_as_list = "clue"
...
def validate_menu_choice(menu_choice):
    ...
    if number == "1":
        ...
        print(
            "WORD GAME - The game selects Word at"
            " random from the Daily Dictionary Dataset")
        print(
            "and displays the definition of that word.\n")
        print(
            "You'll then have to determine the"
            " word from its definition description.\n")
        print("That sounds pretty tough, right?")
        print(
            "Don't worry, to make the task easier we will"
            " display the mixed up letters of the word as a clue.\n")

        print(
            "DEFINITION'S GAME:The game will"
            " display a definition and three words.\n")
        print("One of the words will match the Definition."
              " The other two will be random words from our Dictionary.\n")
        print("Can you match the Definiton to the correct word?\n")

        print("DICTIONARY SEARCH:")
        print("The search function allows"
              " you to look up a word in our dictionary.")
        print(f"There are {row_count} words in our Dataset.",
              " If your word is included then the",
              " search function will return the Definition.\n\n")
        a = input("To return to the main menu,"
                  " please type type any letter and then press enter:\n")
        if a:
            cls()
            main_menu()
        elif a == "y":
...
def word_game(row_ref_start):
    cls()
# The Word game function displays the Game text,
#  shuffled word, meaning and the user input.
...
# This user answer will then be passed to the validate user answer function.
    while True:
        ...
        global word
        word = random_row[0]
        # assigns the value of column 0
        #  (words column) in the sheet to the word variable
        global meaning
        meaning = random_row[1]
        # assigns the value of column 1 (meanings column)
        #  in the sheet to the meaning variable
        global word_as_list
        word_as_list = list(word)
        # converts out word to a list so it
        #  can be shuffled and displayed as a clue
        random.shuffle(word_as_list)
        # shuffles the word/clue

        out_str = " "
        clue = out_str.join(word_as_list)
        # puts the shuffled word back together again as a clue for the user.

        word_as_list = list(word)
        # Converts our word into a list so it can be shuffled.
        random.shuffle(word_as_list)
        # Shuffles our word so the user has a
        #  clue of what word they are searching for.
        print(colored(f.renderText("Word Game \n"), 'green'))
        print("We have selected a word or phrase from"
              f" our Dictionary containing {row_count} entries")
        print("Can you guess the word from its Dictionary Definition below?\n")
        print("The letters of the word have been jumbled up but we"
              " capitalised the first letter of the word to help you.\n")
...
def definition_game(row_ref_start):
    cls()
    """
    The Definition game function will create a
    word/definition combo as well as an additonal 2 words.
    The game will display the defintion and 3 words
    and the user will have to choose which word
    ...
    print(f"Definition: {meaning}.\n")
    print("Please choose the correct word to match the Definition:\n")
...
def validate_user_answer(user_answer):
    ...
    # this will account for the users answer being all lowercase
    ...
    if validate_answer == word:
        ...
        # this will return a correct answer if the
        # user doesn't returns a lowercase correct answer
        print(f"Correct! {word}: {meaning}")
        play_again = input("Would you like to play again? y/n:\n")
        if play_again == "y":
            cls()
            ...
            print(f"{play_again} is not a valid input. Please enter y/n.")

    elif validate_answer != word:
        print(f"{user_answer} is incorrect."
              f"The word we are looking for is {word}")
        play_again = input("Would you like to play again? y/n:\n")
        if play_again == "y":
            cls()
            word_game(row_ref_start)
        ...
        user_answer = input("That isn't the corrent word."
...
def validate_user__definition_answer(user_def_answer):
    ...
    if validate_def_answer == word:
        ...
        # this will return a correct answer if
        # the user doesn't returns a lowercase correct answer
        print(f"Correct! {word}: {meaning}")
        play_again = input("Would you like to play again? y/n:\n")
        if play_again == "y":
            cls()
        ...
        print(f"{user_def_answer} is incorrect."
              f" The word we are looking for is {word}")
        play_again = input("Would you like to play again? y/n:\n")
        if play_again == "y":
            cls()
        ...
        user_def_answer = input(
            "That isn't the corrent word."
            " Please try again.\n Enter your answer here:\n")
...
def dictionary_search_loop():
    ...
    the user search input then
    the function will return the row value
    - this will be the word/definition list.
    """

    word_meaning_examples = SHEET.worksheet('word_meaning_examples')
    word_column = word_meaning_examples.col_values(1)
    # The is the list of words from the word column in our sheet

    print(colored(f.renderText("Dictionary Search"), 'red'))
    print("Feel free to search for a word from our Dictionary.")
    ...
    user_search = input("Please search for a word or type"
        ...
        if user_search == "menu":
            ...
            if user_search_capitalize in word_column:
                ...
                if search_again == "y":
                    ...
                    print("invalid input"), dictionary_search_loop()
            else:
                print("Sorry, we unfortunately we don't have that word"
                      " in our Dictionary, please search for another word.\n"),
                dictionary_search_again()
        else:
            print("Sorry, unfortunately we don't have that word in"
                  " our Dictionary, please search for another word.\n"),
            dictionary_search_again()

    except ValueError:
        print("Apologies! That word isn't in our "
              "Dictionary collection, please try again.\n"),
        dictionary_search_again()
...
def dictionary_search_again():
    ...
    """
    This function is a duplicate of the the
    dictionary_search_again() function
    ...
    try:
        if user_search == "menu":
            ...
            if user_search_capitalize in word_column:
                ...
                print("Sorry, unfortunately we don't have that word in our"
        ...
        print("Apologies! That word isn't in our Dictionary"
